...
export async function getProjectDetail(domainStr: string, noOption?: boolean) {
    const { project_name, primary_logo, favicon, industry, description, secondary_logo, hasGotProjectDetail, knowledge_base } = getEditDomainStoreData()
    if (hasGotProjectDetail.value) return
    try {
        const res = await instance.post("/askai/project/get_base_info", { domain: domainStr }, noOption ? {} : instanceOptions) as Record<string, any>
...
export async function updateProjectDetail(domainStr: string) {
    const { project_name, primary_logo, favicon, industry, description, secondary_logo } = getEditDomainStoreData()
    try {
        await instance.post("/askai/project/modify_base_info", {
            domain: domainStr,
...
export async function createKnowledgeBase(domain: string) {
    try {
        ...
        await instance.post("/askai/project/create_knowledge_base_file", {
            domain,
...
export async function updateKnowledgeBase(domain: string) {
    const { activeKnowledge, hasGotProjectDetail, knowledgeTitle, knowledgeContent } = getEditDomainStoreData()
    try {
...
export function closeKnowledgeModal() {
    const { knowledgeContent, knowledgeTitle, knowledgeModalShow, isEditKnowledge } = getEditDomainStoreData()
...
export async function deleteKnowledgeBase(domain: string, knowledge: KnowledgeBase) {
    ...
    try {
        await instance.post("/askai/project/remove_knowledge_base_file", { domain, kid: knowledge.kid }, instanceOptions)
        hasGotProjectDetail.value = false
