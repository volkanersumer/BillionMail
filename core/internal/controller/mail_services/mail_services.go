...
func SyncBccToPostfix(ctx context.Context) error {
	...
	// configPath
	postfixConfigDir := "/opt/Billion-Mail/conf/postfix/conf"
	...
	if !gfile.Exists(postfixConfigDir) {
		return gerror.Newf("Postfix configuration directory does not exist : %s", postfixConfigDir)
...
// Checks if the string is in domain format
func isDomain(str string) bool {
	domainRegex := regexp.MustCompile(`^(?:[a-zA-Z0-9](?:[a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z]{2,}$`)
	return domainRegex.MatchString(str)
}

// Extract the domain name from the address
