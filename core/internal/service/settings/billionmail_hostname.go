...
func CheckHostname() {
	...
	g.Log().Warning(ctx, "billionmail_hostname  start 111111111")
	...
	if gfile.Exists(flagFile) {
		...
		g.Log().Warning(ctx, "flagFile content 111111111: ", content)
	} else {
		g.Log().Warning(ctx, "flagFile not exists 111111111")
	}
	// ----------------------------------------------------------------------

	if hostname == "" || hostname == "mail.example.com" {
		_ = os.Remove(flagFile)
		g.Log().Warning(ctx, "No hostname 111111111")
		return
	}
	...
	if err != nil {
		g.Log().Warning(ctx, "No serverIP 111111111")
		return
	}
	...
	g.Log().Warning(ctx, "Valid   111111111", Valid)
	if Valid {
